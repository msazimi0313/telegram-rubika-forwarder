...
try:
    ...
    print("خطا: یکی از متغیرهای محیطی (API_ID, API_HASH, SESSION_STRING) تنظیم نشده است.")
    exit()
...
telegram_client = TelegramClient(SESSION_STRING, API_ID, API_HASH)
...
async def handle_deleted_message(event):
    for msg_id in event.deleted_ids:
        if msg_id in message_map:
            rubika_id = message_map.pop(msg_id) # حذف از دفترچه
            save_data_to_file('message_map.json', message_map)
            
            # پیدا کردن کانال مقصد
            # این بخش نیاز به بهبود دارد چون ما مقصد را اینجا نداریم
            # فعلا از اولین کانال روبیکا در نقشه استفاده می کنیم
